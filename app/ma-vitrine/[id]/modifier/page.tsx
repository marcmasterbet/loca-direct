...
export default function ModifierVitrinePage() {
  ...
  const [data, setData] = useState<any>({
    titre: '', type_logement: 'Appartement', ville: '', code_postal: '',
    quartier: '', surface: '', nb_pieces: '', nb_chambres: '', capacite: '',
    etage: '', description_courte: '', description_longue: '', prix_nuit: '',
    prix_semaine: '', prix_mois: '', caution: '', charges_incluses: false,
    taxe_sejour: false, disponible_du: '', type_location: 'Courte durée',
    duree_min: '1 nuit', equipements: [] as string[], photos: [] as string[],
    whatsapp: '', regles: '', animaux: 'Non', fumeurs: 'Non', fetes: 'Non',
      ...
      try {
        ...
        if (res.ok && result.vitrine) {
          ...
          setData({
            ...
            whatsapp: v.whatsapp || '',
            regles: v.regles || '',
            animaux: v.animaux || 'Non',
            fumeurs: v.fumeurs || 'Non',
            fetes: v.fetes || 'Non',
            enfants: v.enfants || 'Oui',
...
function Label({ children }: { children: React.ReactNode }) {
  return <label style={{ fontSize: 12, fontWeight: 600, color: '#1F2937', marginBottom: 6, display: 'block' }}>{children}</label>
}
