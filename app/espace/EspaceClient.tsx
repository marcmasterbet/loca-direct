  ...
  return (
    <div style={{ background: GRAY, minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '24px 16px 100px' }}>
        ...
        {activeTab === 'accueil' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {/* Carte bienvenue */}
            <div style={{ background: `linear-gradient(135deg, ${ORANGE_LIGHT} 0%, ${WHITE} 100%)`, border: `1px solid ${BORDER}`, borderRadius: 20, padding: 28 }}>
              <p style={{ fontSize: 11, color: ORANGE, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 8 }}>Bienvenue</p>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: TEXT, marginBottom: 8 }}>Bonjour 👋</h1>
              <p style={{ fontSize: 13, color: TEXT_DIM, marginBottom: 20, lineHeight: 1.6 }}>{user.email}</p>
              {(user.est_hebergeur || user.est_prestataire) && (
                <button onClick={() => setActiveTab('publier')} style={{ background: ORANGE, color: WHITE, borderRadius: 12, padding: '12px 22px', fontSize: 14, fontWeight: 700 }}>
                  + Publier
                </button>
              )}
            </div>
            ...
            {user.est_hebergeur && (
              <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20 }}>
                <h2 style={{ fontSize: 14, fontWeight: 700, color: TEXT, marginBottom: 16 }}>
                  Mes vitrines ({vitrines.length})
                </h2>

                {vitrines.length === 0 ? (
                  <p style={{ fontSize: 13, color: TEXT_DIM }}>Vous n'avez pas encore publié de vitrine.</p>
                ) : (
                    ...
                    {vitrines.map((v) => {
                      ...
                      return (
                        <div key={v.id} style={{ border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                          ...
                          {/* En-tête carte — cliquable vers la page publique de la vitrine */}
                          <Link
                            href={`/vitrine/${v.id}`}
                            style={{ display: 'flex', gap: 12, padding: 14, alignItems: 'flex-start' }}
                          >
                            {/* Photo miniature */}
                            <div style={{ width: 56, height: 56, borderRadius: 10, overflow: 'hidden', flexShrink: 0, background: GRAY }}>
                              {v.photos?.[0] ? (
                          ...
                          {/* Lien vitrine */}
                          {canShare && (
                            <div style={{ margin: '0 14px 10px', display: 'flex', alignItems: 'center', gap: 8, background: GRAY, borderRadius: 8, padding: '8px 12px' }}>
                              <span style={{ fontSize: 11, color: TEXT_DIM, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                🔗 {vitrineLink}
                              </span>
                            </div>
                          )}
                          ...
                          <div style={{ padding: '0 14px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                            ...
                            {canManage && (
                              <button
                                ...
                                style={{
                                  ...
                                  fontSize: 13,
                                  fontWeight: 600,
                                  background: v.statut === 'active' ? RED_LIGHT : GREEN_LIGHT,
                                  color: v.statut === 'active' ? '#DC2626' : GREEN,
                                  border: `1px solid ${v.statut === 'active' ? '#FECACA' : '#BBF7D0'}`,
                            ...
                            {canShare && (
                              <div style={{ display: 'flex', gap: 8 }}>
                                <a
                                  href={`https://wa.me/?text=${encodeURIComponent(shareText + ' ' + vitrineLink)}`}
                                  target="_blank"
                                  rel="noreferrer"
                                  style={{
                                    flex: 1,
                                ...
                                <a
                                  href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(vitrineLink)}`}
                                  target="_blank"
                                  rel="noreferrer"
                                  style={{
                                    flex: 1,
                                ...
                                <button
                                  ...
                                  style={{
                                    ...
                                    fontSize: 12,
                                    fontWeight: 600,
                                    border: `1px solid ${copiedVitrineId === v.id ? GREEN : BORDER}`,
                                  }}
                                >
                                  {copiedVitrineId === v.id ? '✓ Copié !' : '🔗 Copier'}
                                </button>
                              </div>
                            )}
                          </div>
            ...
            {/* Mes prestations */}
            {user.est_prestataire && (
              <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20 }}>
                <h2 style={{ fontSize: 14, fontWeight: 700, color: TEXT, marginBottom: 16 }}>
                  🛠️ Mes prestations ({prestataires.length})
                </h2>
                ...
                {prestataires.length === 0 ? (
                  <div>
                    ...
                    <Link
                      href="/espace/devenir-prestataire"
                      style={{ display: 'inline-block', background: ORANGE, color: WHITE, borderRadius: 10, padding: '10px 18px', fontSize: 13, fontWeight: 700 }}
                    >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {prestataires.map((p) => {
                      ...
                      const prestataireLink = `https://loca-direct.fr/prestataires/${p.id}`
                      const canShare = p.statut === 'active'

                      return (
                        <div key={p.id} style={{ border: `1px solid ${BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
                          <Link
                            href={canShare ? `/prestataires/${p.id}` : '#'}
                            style={{ display: 'flex', gap: 12, padding: 14, alignItems: 'flex-start', cursor: canShare ? 'pointer' : 'default' }}
                          >
                            <div style={{ width: 56, height: 56, borderRadius: 10, overflow: 'hidden', flexShrink: 0, background: GRAY }}>
                              {p.flyer_url ? (
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <p style={{ fontSize: 12, color: TEXT_DIM, marginBottom: 6 }}>{p.activite} · {p.ville}{p.region ? ` (${p.region})` : ''} · 👁 {p.nb_vues} vues</p>
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: statut.bg, borderRadius: 20, padding: '3px 10px' }}>
                                <span style={{ width: 6, height: 6, borderRadius: '50%', background: statut.dot, display: 'inline-block' }} />
                                <span style={{ fontSize: 11, fontWeight: 600, color: statut.color }}>{statut.label}</span>
                              </span>
                          ...
                          {p.motif_refus && (
                            <div style={{ margin: '0 14px 10px', background: RED_LIGHT, borderRadius: 8, padding: '8px 12px' }}>
                              <p style={{ fontSize: 12, color: '#DC2626' }}>⚠️ {p.motif_refus}</p>
                            </div>
                          )}
                          ...
                          <div style={{ padding: '0 14px 14px' }}>
                            <Link
                              href={`/espace/devenir-prestataire/${p.id}/modifier`}
                              style={{
                                display: 'block',
                                textAlign: 'center',
                                padding: '10px 12px',
            ...
            {user.est_voyageur && (
              <button
                ...
                style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20, display: 'block', textAlign: 'left', width: '100%' }}
              >
                <h2 style={{ fontSize: 14, fontWeight: 700, color: TEXT }}>🔍 Rechercher un logement</h2>
              </button>
            )}
          </div>
        )}
        ...
        {activeTab === 'rechercher' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '14px 16px', borderRadius: 12, border: `1px solid ${BORDER}`, fontSize: 15, outline: 'none', background: WHITE }}
            />
                ...
                {filteredVitrines.map((v) => (
                  <Link key={v.id} href={`/vitrine/${v.id}`} style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: 'hidden', display: 'block' }}>
                    <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3', background: GRAY }}>
                      {v.photos?.[0] ? (
                        <img src={v.photos[0]} alt={v.titre} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      ) : (
                        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32 }}>🏠</div>
                      )}
                      <div style={{ position: 'absolute', top: 8, left: 8 }}>
                        {v.statut === 'active' && <span style={{ background: GREEN, color: WHITE, borderRadius: 20, padding: '3px 9px', fontSize: 10, fontWeight: 700 }}>✅ Disponible</span>}
                    ...
                    <div style={{ padding: 12 }}>
                      <p style={{ fontSize: 12, color: TEXT_DIM, marginBottom: 2 }}>{v.type_logement} · {v.ville}</p>
                      <p style={{ fontSize: 13, fontWeight: 700, color: TEXT, marginBottom: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.titre}</p>
        ...
        {/* ONGLET PUBLIER — sélecteur Logement / Service */}
        {activeTab === 'publier' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <Link
              ...
              style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24, display: 'flex', alignItems: 'center', gap: 16 }}
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 13, color: TEXT_DIM }}>Publiez gratuitement votre logement et louez sans commission.</p>
              </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 13, color: TEXT_DIM }}>Conciergerie, ménage, photographe... rejoignez l'annuaire des prestataires.</p>
              </div>
              <span style={{ fontSize: 18, color: TEXT_DIM }}>→</span>
            </Link>
          </div>
            ...
            <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                <input
                  ...
                  style={{ flex: 1, padding: '10px 14px', borderRadius: 10, border: `1px solid ${BORDER}`, fontSize: 14, outline: 'none' }}
                />
                <button onClick={handleAddAlerte} disabled={addingAlerte} style={{ background: ORANGE, color: WHITE, borderRadius: 10, padding: '10px 16px', fontSize: 13, fontWeight: 700, opacity: addingAlerte ? 0.6 : 1 }}>
        ...
        {activeTab === 'profil' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20 }}>
              ...
              <Row
                label="Profil"
                value={[user.est_voyageur && 'Voyageur', user.est_hebergeur && 'Hébergeur', user.est_prestataire && 'Prestataire']
                  .filter(Boolean)
                  .join(' + ')}
              />
            ...
            <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: GRAY, borderRadius: 10, padding: '10px 14px' }}>
                ...
                <button
                  onClick={async () => { await navigator.clipboard.writeText(refLink); setCopiedRef(true); setTimeout(() => setCopiedRef(false), 2000) }}
                  style={{ background: copiedRef ? GREEN : ORANGE, color: WHITE, borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}
                >
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: WHITE, borderTop: `1px solid ${BORDER}`, display: 'flex', zIndex: 20, paddingBottom: 'env(safe-area-inset-bottom)', boxShadow: '0 -4px 20px rgba(0,0,0,0.06)' }}>
        {navItems.map((item) => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} style={{ flex: 1, padding: '10px 4px 8px', background: 'transparent', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            {item.primary ? (
              ...
              <span style={{ fontSize: 22, opacity: activeTab === item.id ? 1 : 0.4 }}>{item.icon}</span>
            )}
            <span style={{ fontSize: 10, color: activeTab === item.id ? ORANGE : TEXT_DIM, fontWeight: activeTab === item.id ? 700 : 400 }}>{item.label}</span>
          </button>
        ))}
      </div>
    </div>
