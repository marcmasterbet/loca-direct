...
export default function ModifierPrestatairePage() {
  ...
  useEffect(() => {
    const fetchPrestataire = async () => {
      try {
        const res = await fetch(`/api/get-prestataire?id=${id}`)
        const result = await res.json()
        if (res.ok && result.prestataire) {
          const p = result.prestataire
          setData({
            nom: p.nom || '',
            prenom: p.prenom || '',
            siret: p.siret || '',
            ...
            activite: p.activite || 'Conciergerie',
            description: p.description || '',
            telephone: p.telephone || '',
            whatsapp: p.whatsapp || '',
            site_web: p.site_web || '',
            tarif_horaire: p.tarif_horaire || '',
            sur_devis: p.sur_devis || false,
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/update-prestataire', {
        method: 'POST',
  ...
  return (
    <div style={{ background: GRAY, minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px 60px' }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 800, color: TEXT, marginBottom: 4 }}>Modifier mon profil prestataire</h1>
...
function Step5({ data, set }: any) {
  ...
  return (
    <div>
      <StepTitle>🖼️ Flyer / Visuel principal</StepTitle>
      {data.flyer_url ? (
        ...
        <div
          onClick={() => !uploadingFlyer && document.getElementById('flyer-upload-modifier')?.click()}
          style={{ aspectRatio: '16/9', borderRadius: 12, border: `2px dashed ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexDirection: 'column', gap: 6, marginBottom: 20 }}
        >
          {uploadingFlyer ? (
            <span style={{ fontSize: 13, color: TEXT_DIM }}>Envoi en cours...</span>
          ) : (
            <>
              <span style={{ fontSize: 28, color: TEXT_DIM }}>+</span>
              <span style={{ fontSize: 12, color: TEXT_DIM }}>Ajouter un flyer ou visuel</span>
            </>
          )}
        </div>
      )}
      <input id="flyer-upload-modifier" type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) handleUploadFlyer(f) }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
        ...
        {data.photos.length < 10 && (
          <div onClick={() => !uploadingPhoto && document.getElementById('photo-upload-modifier-presta')?.click()} style={{ aspectRatio: '1', borderRadius: 10, border: `2px dashed ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexDirection: 'column', gap: 4 }}>
      ...
      <input id="photo-upload-modifier-presta" type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) handleUploadPhoto(f) }} />
    </div>
  )
}
