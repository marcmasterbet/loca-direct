...
function Step1({ data, set }: any) {
  return (
    <div>
      ...
      <Label>SIRET</Label>
      <Input value={data.siret} onChange={v => set('siret', v)} placeholder="123 456 789 00012" />
      <Label>Adresse du siège</Label>
      <Input value={data.adresse_siege} onChange={v => set('adresse_siege', v)} placeholder="12 rue de la Paix" />
      <Label>Ville</Label>
...
function Step2({ data, set }: any) {
  return (
    <div>
      ...
      <Select value={data.activite} onChange={v => set('activite', v)} options={ACTIVITES} />
      <Label>Description de votre activité</Label>
      <Textarea value={data.description} onChange={v => set('description', v.slice(0, 600))} placeholder="Décrivez vos services, votre expérience, votre zone d'intervention..." rows={6} />
...
function Step3({ data, set }: any) {
  return (
    <div>
      ...
      <Checkbox checked={data.sur_devis} onChange={v => set('sur_devis', v)} label="Sur devis (pas de tarif fixe)" />
      {!data.sur_devis && (
        <>
          <Label>Tarif horaire (€)</Label>
          <Input type="number" value={data.tarif_horaire} onChange={v => set('tarif_horaire', v)} placeholder="35" />
        </>
      )}
    </div>
  )
...
function Step4({ data, set }: any) {
  return (
    <div>
      ...
      <Input value={data.telephone} onChange={v => set('telephone', v)} placeholder="+33 6 12 34 56 78" />
  ...
  return (
    <div>
      ...
      {data.flyer_url ? (
        ...
        <div
          onClick={() => !uploadingFlyer && document.getElementById('flyer-upload')?.click()}
      ...
      <input id="photo-upload" type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) handleUploadPhoto(f) }} />
      ...
      <div style={{ background: ORANGE_LIGHT, border: `1px solid ${ORANGE}`, borderRadius: 12, padding: 16, marginTop: 10 }}>
        <p style={{ fontSize: 13, color: ORANGE, lineHeight: 1.6 }}>
          ...
          Vous recevrez un email dès qu'il sera en ligne.
        </p>
      </div>
    </div>
  )
