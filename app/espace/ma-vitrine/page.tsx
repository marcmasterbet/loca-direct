...
function Step1({ data, set, handlePaysChange }: any) {
  ...
  return (
    <div>
      ...
      <Input value={data.numero_enregistrement} onChange={(v: string) => set('numero_enregistrement', v)} placeholder="Ex: 7500100123456" />
      ...
      <select
        ...
        style={{ width: '100%', padding: '11px 14px', borderRadius: 10, border: `1px solid ${BORDER}`, fontSize: 14, outline: 'none', color: TEXT, marginBottom: 14, background: WHITE }}
      >
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
        ...
        {data.photos.length < 10 && (
          <div onClick={() => !uploading && document.getElementById('photo-upload')?.click()} style={{ aspectRatio: '1', borderRadius: 10, border: `2px dashed ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexDirection: 'column', gap: 4 }}>
  ...
  return (
    <div>
      ...
      <div style={{ background: GRAY, borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 13, color: TEXT_DIM }}>{data.type_logement} · {data.ville}{data.region ? `, ${data.region}` : ''} · {paysFlags[data.pays]} {data.pays} · {data.surface}m²</p>
        ...
        {data.numero_enregistrement && <p style={{ fontSize: 12, color: TEXT_DIM, marginTop: 4 }}>N° enregistrement : {data.numero_enregistrement}</p>}
      </div>
      <div style={{ background: ORANGE_LIGHT, border: `1px solid ${ORANGE}`, borderRadius: 12, padding: 16 }}>
        <p style={{ fontSize: 13, color: ORANGE, lineHeight: 1.6 }}>
          ⏳ Votre annonce sera vérifiée par notre équipe avant publication. Vous recevrez un email dès qu'elle sera en ligne (généralement sous 24-48h).
        </p>
      </div>
    </div>
  )
