...
function Step1({ data, set }: any) {
  return (
    <div>
      ...
      <p style={{ fontSize: 11, color: TEXT_DIM }}>La région est recalculée automatiquement à partir du code postal.</p>
    </div>
  )
}
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
        ...
        {data.photos.length < 10 && (
          <div
            onClick={() => !uploadingPhoto && document.getElementById('photo-upload-modifier')?.click()}
      ...
      <input id="photo-upload-modifier" type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) handleUploadPhoto(f) }} />
    </div>
  )
}
