...
/**
 * Déduit la région française à partir d'un code postal.
 * Gère la Corse (2A/2B logiquement remontée via "20") et les DOM-TOM (3 chiffres).
 * Retourne null si le code postal est invalide ou non reconnu.
 */
export function getRegionFromCodePostal(codePostal: string): string | null {
  ...
  // DOM-TOM : codes à 3 chiffres commençant par 97x ou 98x
  if (/^9[78]\d/.test(cp)) {
  ...
  if (cp.startsWith('20')) {
    // 200-201 = 2A (Corse-du-Sud), 202-209 = 2B (Haute-Corse), approximation usuelle
    const num = parseInt(cp.slice(0, 3), 10)
    return num <= 201 ? DEPARTEMENT_TO_REGION['2A'] : DEPARTEMENT_TO_REGION['2B']
  }

  // Métropole : 2 premiers chiffres = département
